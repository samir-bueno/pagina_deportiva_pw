...
const AgregarProducto = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Cargar talles
      const sizesResponse = await fetch('http://127.0.0.1:5003/sizes');
      const sizesData = await sizesResponse.json();
      setSizes(sizesData);

      // Cargar colores
      const colorsResponse = await fetch('http://127.0.0.1:5003/colors');
      const colorsData = await colorsResponse.json();
      setColors(colorsData);

      // Cargar marcas
      const brandsResponse = await fetch('http://127.0.0.1:5003/brands');
      const brandsData = await brandsResponse.json();
      setBrands(brandsData);

      // Cargar años
      const yearsResponse = await fetch('http://127.0.0.1:5003/years');  // Verifica la URL correcta
